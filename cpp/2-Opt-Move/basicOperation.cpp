...
bool p_isIntersectSegments(tom_points *points, int edge1s, int edge1e, int edge2s, int edge2e);
int isAOnLine(tom_point point1,  tom_point point2, tom_point a);
tom_point p_getPointOfArray(tom_points *points, int index);
int isIntersectOrderedSegs(tom_point e1point1, tom_point e1point2, tom_point e2point1, tom_point e2point2);
...
bool p_isSimplePolygon(tom_polygon *poly, tom_points *points)
{
    int nOfPolyPoints = p_numOfPolyElems(poly);
    int idx1, idx2;
    bool isSPG = true;
...
bool p_isIntersectEdge(tom_points *points, int edge1s, int edge1e, int edge2s, int edge2e){
...
//调用该函数前check了，搭接的情况
int isIntersectOrderedSegs(tom_point e1point1, tom_point e1point2, tom_point e2point1, tom_point e2point2){
    int e1sPosOfEdge2, e1ePosOfEdge2, e2sPosOfEdge1, e2ePosOfEdge1;
    int res1, res2, ans;
    e1sPosOfEdge2 = isAOnLine(e2point1, e2point2, e1point1);
    e1ePosOfEdge2 = isAOnLine(e2point1, e2point2, e1point2);

    //if in same side
    if((res1 = e1sPosOfEdge2 * e1ePosOfEdge2) == 1){    //1 ~
        ans = false; //不相交
    }
    else{
        e2sPosOfEdge1 = isAOnLine(e1point1, e1point2, e2point1);
        e2ePosOfEdge1 = isAOnLine(e1point1, e1point2, e2point2);
        if ( (res2 = e2sPosOfEdge1 * e2sPosOfEdge1) == 1 ){ // ~ 1
            ans = false;    //不相交
        }else if(res1 + res2 == -2){    // -1 -1
            ans = true; //相交
        }else if((e1sPosOfEdge2 + 2*e1ePosOfEdge2 + 4*e2sPosOfEdge1 + 8*e2ePosOfEdge1) == 0){ //-1 0、0 -1、0 ~、~ 0【这里即以后是handle 0 的情况】
...
//其中Point Idx = 1有关的相关相交关系占用n-1的位置、Point Idx = 2有关的相关相交关系占用n-2的位置、……
//每个点只记录idx大于这个点的相交关系的header。【每个指针指向的结构体的prev、next才是记录这条边以西恶劣】
//备注：(2*n - i1)*(i1 - 1)/2 = (n-1) + (n-2) + ... + (n-(i1-1))
tom_sectP* getHeadSectOfEdgeInArray(tom_sectP* sectArray, int pIdx1, int pIdx2, int numOfPoints){
    ...
    int offset = (2*numOfPoints - minpIdx)*(minpIdx - 1)/2 + maxpIdx - minpIdx - 1;
